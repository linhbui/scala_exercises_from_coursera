...
object Anagrams {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val mapY = y.toMap
    def subtractRec(x: Occurrences): Occurrences = x match {
      case Nil => Nil
      case (char, count)::xs => mapY.get(char) match {
        case None => (char, count)::subtractRec(xs)
        case Some(countY) => if (count == countY) subtractRec(xs) else (char, count - countY)::subtractRec(xs)
      }
    }
